...
Процедура УстановитьЗапретФоновыхЗаданий() Экспорт
	ДописатьВЛогФайл("*** Начало выполнения", 1);
	...
	Для Каждого ИБ Из Дескриптор.ConnectToWorkProcess.GetInfoBases() Цикл
		Если Не ИБ.ScheduledJobsDenied Тогда
			ДописатьВЛогФайл("Устанавливается флаг запрета ФЗ на ИБ "+ИБ.DBName, 2);
...
Процедура ДописатьВЛогФайл(СтрокаВЛогФайл, Уровень) Экспорт
	...
	//Если Настройки.дбг >= Уровень Тогда
	Сообщить(Рапорт);
	//КонецЕсли;
...
Функция   ПолучитьСоединениеСКластером()
	...
	ДописатьВЛогФайл("Создание COM-коннектора <"+ Соединение + ">", 2);
...
//			wp = WorkingProcesses.GetValue(j);
			...
			Если wp.Running = 1 Тогда
				...
				Исключение
					...
					Продолжить;
				КонецПопытки;
				ДописатьВЛогФайл("Авторизация в кластере: <"+Настройки.АдминистраторКластера+">", 2);
				Попытка
				...
				Исключение
					...
					Продолжить;
				КонецПопытки;
				ДописатьВЛогФайл("Авторизация в ИБ: <"+Настройки.АдинистраторИнформационнойБазы+">", 2);
				Попытка
...
Процедура ПроверитьТекущегоПользователя()
  LegalUserList = "tasker, goblin, sukhikh";
